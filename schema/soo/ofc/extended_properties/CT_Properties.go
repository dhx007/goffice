...
func (m *CT_Properties) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	if m.PresentationFormat != nil {
		...
		goffice.AddPreserveSpaceAttr(&sePresentationFormat, *m.PresentationFormat)
	...
	if m.HyperlinkBase != nil {
		...
		goffice.AddPreserveSpaceAttr(&seHyperlinkBase, *m.HyperlinkBase)
	...
	if m.Application != nil {
		...
		goffice.AddPreserveSpaceAttr(&seApplication, *m.Application)
		e.EncodeElement(m.Application, seApplication)
	}
	if m.AppVersion != nil {
		seAppVersion := xml.StartElement{Name: xml.Name{Local: "AppVersion"}}
		goffice.AddPreserveSpaceAttr(&seAppVersion, *m.AppVersion)
...
lCT_Properties:
	for {
		...
		case xml.StartElement:
			...
			default:
				goffice.Log("skipping unsupported element on CT_Properties %v", el.Name)
