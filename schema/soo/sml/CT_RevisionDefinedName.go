...
func (m *CT_RevisionDefinedName) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	if m.Formula != nil {
		...
		goffice.AddPreserveSpaceAttr(&seformula, *m.Formula)
		e.EncodeElement(m.Formula, seformula)
	}
	if m.OldFormula != nil {
		seoldFormula := xml.StartElement{Name: xml.Name{Local: "ma:oldFormula"}}
		goffice.AddPreserveSpaceAttr(&seoldFormula, *m.OldFormula)
...
lCT_RevisionDefinedName:
	for {
		...
		case xml.StartElement:
			...
			default:
				goffice.Log("skipping unsupported element on CT_RevisionDefinedName %v", el.Name)
